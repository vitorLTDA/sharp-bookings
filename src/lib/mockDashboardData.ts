...
function generateAppointments(): DashboardAppointment[] {
  ...
  const names = ['John Doe', 'Mike Smith', 'Chris Johnson', 'Alex Brown', 'Sam Wilson', 'Tom Davis'];
  ...
  for (let i = 0; i < 25; i++) {
    ...
    appointments.push({
      ...
      createdAt: new Date(date.getTime() - 86400000 * Math.random() * 7).toISOString(),
    });
  }
...
function generateSubscriptions(): Subscription[] {
  ...
  const names = ['Premium Customer', 'Regular Joe', 'VIP Client', 'Business Account'];
...
export function getMetrics() {
  const totalBookings = mockAppointments.filter(a => a.status !== 'canceled').length;
  ...
  const bookingRevenue = mockAppointments
    .filter(a => a.status === 'paid')
    .length * 35; // Assuming $35 per booking
  ...
  return {
    ...
    unpaidAppointments: mockAppointments.filter(a => a.status === 'unpaid').length,
    canceledAppointments: mockAppointments.filter(a => a.status === 'canceled').length,
  };
}
